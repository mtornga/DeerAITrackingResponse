#!/usr/bin/env bash
#
# Evaluator Agent - Shell Wrapper
#
# Usage:
#   ./agents/evaluator/run.sh
#   ./agents/evaluator/run.sh --dry-run
#
...
#   LOG_DIR       - Log output directory (default: ${REPO_DIR}/runs/logs/evaluator)
...
LOG_DIR="${LOG_DIR:-${REPO_DIR}/runs/logs/evaluator}"
...
if command -v timeout >/dev/null 2>&1; then
    set +e
    timeout "${SHELL_TIMEOUT}" "${PYTHON_BIN}" agents/evaluator/run.py "$@"
    exit_code=$?
    set -e
    ...
    set +e
    "${PYTHON_BIN}" agents/evaluator/run.py "$@"
    exit_code=$?
    set -e
fi

echo "Evaluator finished with exit code ${exit_code}"
exit ${exit_code}
