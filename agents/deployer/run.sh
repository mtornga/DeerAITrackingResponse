#!/usr/bin/env bash
#
# Deployer Agent - Shell Wrapper
#
# Usage:
#   ./agents/deployer/run.sh
#   ./agents/deployer/run.sh --dry-run
#
...
#   LOG_DIR       - Log output directory (default: ${REPO_DIR}/runs/logs/deployer)
...
LOG_DIR="${LOG_DIR:-${REPO_DIR}/runs/logs/deployer}"
...
if command -v timeout >/dev/null 2>&1; then
    set +e
    timeout "${SHELL_TIMEOUT}" "${PYTHON_BIN}" agents/deployer/run.py "$@"
    exit_code=$?
    set -e
    ...
    set +e
    "${PYTHON_BIN}" agents/deployer/run.py "$@"
    exit_code=$?
    set -e
fi

echo "Deployer finished with exit code ${exit_code}"
exit ${exit_code}
