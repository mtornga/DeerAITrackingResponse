#!/usr/bin/env bash
#
# Training Orchestrator - Shell Wrapper
#
# Usage:
#   ./agents/training-orchestrator/run.sh
#   ./agents/training-orchestrator/run.sh --dry-run
#
...
#   LOG_DIR       - Log output directory (default: ${REPO_DIR}/runs/logs/training_loop)
...
LOG_DIR="${LOG_DIR:-${REPO_DIR}/runs/logs/training_loop}"
...
if command -v timeout >/dev/null 2>&1; then
    set +e
    timeout "${SHELL_TIMEOUT}" "${PYTHON_BIN}" agents/training-orchestrator/run.py "$@"
    exit_code=$?
    set -e
    ...
    set +e
    "${PYTHON_BIN}" agents/training-orchestrator/run.py "$@"
    exit_code=$?
    set -e
fi

echo "Training Orchestrator finished with exit code ${exit_code}"
exit ${exit_code}
