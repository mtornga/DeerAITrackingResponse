#!/usr/bin/env bash
#
# Queue Enricher Agent - Shell Wrapper
#
# Usage:
...
#   LOG_DIR       - Log output directory (default: ${REPO_DIR}/runs/logs/queue_enricher)
#   SHELL_TIMEOUT - Shell-level timeout in seconds (default: 600 = 10 min)
#
...
LOG_DIR="${LOG_DIR:-${REPO_DIR}/runs/logs/queue_enricher}"
SHELL_TIMEOUT="${SHELL_TIMEOUT:-600}"
...
if command -v timeout >/dev/null 2>&1; then
    set +e
    timeout "${SHELL_TIMEOUT}" "${PYTHON_BIN}" agents/queue-enricher/run.py "$@"
    exit_code=$?
    set -e
    ...
    set +e
    "${PYTHON_BIN}" agents/queue-enricher/run.py "$@"
    exit_code=$?
    set -e
fi

echo "Queue Enricher finished with exit code ${exit_code}"
exit ${exit_code}
